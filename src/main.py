...
args.output_folder = os.path.join("runs", args.exp_name, start_time.strftime("%Y-%m-%d_%H-%M-%S"))
...
######################################### DATASETS #########################################
logging.debug(f"Loading datasets from folder {args.root_path}")

query_train_set = datasets.QueryDataset(args.root_path, args.train_g, args.train_q, args.output_folder)
logging.info(f"Train query set: {query_train_set.info}")

whole_train_set = datasets.WholeDataset(args.root_path, args.train_g, args.train_q)
logging.info(f"Train whole set: {whole_train_set.info}")

whole_val_set = datasets.WholeDataset(args.root_path, args.val_g, args.val_q)
...
if args.grl:
    grl_dataset = grl_util.GrlDataset(args.root_path, args.grl_datasets.split("+"))
else:
    grl_dataset = None
...
######################################### TEST on TEST SET #########################################
best_model_state_dict = torch.load(os.path.join(args.output_folder, "best_model.pth"))["state_dict"]
